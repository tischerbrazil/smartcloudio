...
//Deprecate gt-geojson and replace with Jackson based gt-geojsondatastore
// FIXME

@SuppressWarnings("serial")
@Path("/oauth2")
public class PolygonEndpolygon implements Serializable {
		...
		Response resposta = null;
			...
			try {
				...
				resposta = Response.created(
						UriBuilder.fromResource(PolygonEndpolygon.class).path(String.valueOf(entity.getId())).build())
						.build();
	...
	public String create(
			...
			throws ParseException, NoSuchAuthorityCodeException, 
			FactoryException, IOException, SchemaException,
		...
		try {
			...
			String consultaxSINGLE = "select " 
			        ...
			        + "geometrias0_.sigman,"
			        + "geometrias0_.area,"
			        + "public.st_transform(public.st_geomfromtext(public.st_astext(geometrias0_.geometrypolygon), " +  layerSRID + "), " +  appSRID + ")  as geometrypolygon,"
			        ...
			        + "WHERE geometrias0_.geometrypolygon is not null and geometrias0_.enabled = true  "
			        + "and geometrias0_.situacao > 0 AND geometrias0_.layer_id=" + layerId
		            + " AND public.st_srid(geometrias0_.geometrypolygon) = " + layerSRID ;
    ...
    + "AND  public.ST_Intersects(geometrias0_.geometrypolygon,  "
	+ "public.ST_Transform(public.st_envelope(public.ST_GeomFromText('LINESTRING(" + x1 + " " + y1 + "," + x2 + " " + y2
	+ ")','"  + appSRID.toString() +  "'))," + layerSRID.toString() + ")"	
	+ "   )  AND "
			...
			if (results.size() > 0) {
					...
					for (Polygon polygon : results) {
						...
						Integer teste = polygon.getSituacao().intValue();
