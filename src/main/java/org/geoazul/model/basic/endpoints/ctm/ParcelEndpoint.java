...
@Path("/public/parcel")
public class ParcelEndpoint implements Serializable {
	...
	public Response listOne(@PathParam("id") Long id) {
		...
		ParcelEntity surfaceFind = null;

		try {
			
			
			TypedQuery<ParcelEntity> surface = entityManager.createNamedQuery(ParcelEntity.PARCEL_ID, ParcelEntity.class);
			surface.setParameter("id", id);
			surfaceFind = surface.getSingleResult();
			...
			String dtoAsString = mapper.writer(filterProvider).writeValueAsString(surfaceFind);
	...
	public String create(@PathParam("id") String id, GeometryJsonModel geomodel) {
		...
		Response resposta = null;
			...
			try {
				...
				queryApp.setParameter("uuid", Long.valueOf(geomodel.getLayerid()));
				...
				resposta = Response.created(
						UriBuilder.fromResource(ParcelEndpoint.class).path(String.valueOf(entity.getId())).build())
						.build();
	...
	public String create(
		...
		try {
			...
			TypedQuery<Layer> queryApp = entityManager.createNamedQuery(Layer.LAYER_ID, Layer.class);
			queryApp.setParameter("uuid",   layer);
			layerFind = queryApp.getSingleResult();
