...
@Path("/oauth2")
public class LinestringEndlinestring implements Serializable {
	...
	public String create(
		...
		try {
			...
			consultax = "select " 
		      ...
		      //  + "abstractge0_.dtype,"
		        + "abstractge0_.field,"
		        + "public.st_transform(public.st_geomfromtext(public.st_astext(linestringgeo1_.geometry), " +  layerSRID + "), " +  appSRID + ")  as geometry,"		        
		        + "linestringgeo1_.valoroffset "
		        + "from APP_GEOMETRY abstractge0_ " 
		        + "left outer join "
		        + "APP_LINESTRING linestringgeo1_ "
		        + "on abstractge0_.id=linestringgeo1_.id " 
		        + "WHERE linestringgeo1_.geometry is not null and abstractge0_.enabled = true  "
		        + "and abstractge0_.situacao > 0 AND abstractge0_.layer_id=" + layerId
		        + " AND public.st_srid(linestringgeo1_.geometry) = " + layerSRID.toString() 
		        + "  LIMIT 5000 ";
			
			
			
			String consultaxSINGLE = "select " 
			        + "geometrias0_.layer_id,"
			        + "geometrias0_.id,"
			        + "geometrias0_.enabled,"
			        + "geometrias0_.imovel_id,"
			        + "abstractge0_.origin_id,"
			        + "geometrias0_.iconflag,"
			        + "geometrias0_.nome,"
			        + "geometrias0_.parte,"
			        + "geometrias0_.situacao,"
			        + "geometrias0_.field,"
			        + "geometrias0_.geometrypolygon,"
			        + "geometrias0_.valoroffset,"
			        + "geometrias0_.geometrymultipolygon,"
			        + "geometrias0_.altura,"
			        + "geometrias0_.geometrypoint,"
			        + "geometrias0_.metodoLevantamento,"
			        + "geometrias0_.sigmae,"
			        + "geometrias0_.sigmah,"
			        + "geometrias0_.sigman,"
			        + "geometrias0_.area,"
			        + "public.st_transform(public.st_geomfromtext(public.st_astext(geometrias0_.geometrylinestring), " +  layerSRID + "), " +  appSRID + ")  as geometrylinestring,"
			    
			        + "geometrias0_.perimetro,"
			        + "geometrias0_.DTYPE "
			  
			       
			        + "from APP_GEOMETRY geometrias0_ " 
			      
			        + "WHERE geometrias0_.geometrylinestring is not null and geometrias0_.enabled = true  "
			        + "and geometrias0_.situacao > 0 AND geometrias0_.layer_id=" + layerId
		            + " AND public.st_srid(geometrias0_.geometrylinestring) = " + layerSRID ;
					...
					consultax = "select " 
					    ...
					    + "left outer join "
					        + "APP_LINESTRING linestringgeo1_ "
					            + "on abstractge0_.id=linestringgeo1_.id " 
							    + "WHERE linestringgeo1_.geometry is not null and abstractge0_.enabled = true  "
							    + "AND  public.ST_Intersects(linestringgeo1_.geometry,  "
    ...
    + "AND  public.ST_Intersects(geometrias0_.geometrylinestring,  "
	+ "public.ST_Transform(public.st_envelope(public.ST_GeomFromText('LINESTRING(" + x1 + " " + y1 + "," + x2 + " " + y2
	+ ")','"  + appSRID.toString() +  "'))," + layerSRID.toString() + ")"
	+ "   )  AND "
					        
					        + " geometrias0_.situacao > 0 AND geometrias0_.layer_id=" + layerId
				            + " AND public.st_srid(geometrias0_.geometrylinestring) = " + layerSRID ;
					
					}
			
			Query findAllQuery33 = entityManager.createNativeQuery(consultax, Linestring.class);
		
			List<Linestring> results = findAllQuery33.getResultList();
			
			if (results.size() > 0) {
								
					// ====================================================
					SimpleFeatureTypeBuilder tb = new SimpleFeatureTypeBuilder();
					tb.setName("Point Geometry");
					try {
						tb.add("geometry", Linestring.class, CRS.decode("EPSG:" + appSRID.toString()));
					...
					for (Linestring linestring : results) {
						fb = new SimpleFeatureBuilder(schema);
						fb.add(linestring.getGeometry());
						fb.add(linestring.getNome());
						fb.add(linestring.getSituacao().intValue());
						fc.add(fb.buildFeature(linestring.getId().toString()));
					}
